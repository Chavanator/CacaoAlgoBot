...
predicciones_file = 'predicciones.csv'
metricas_file = 'metricas.csv'
resultados_file = 'resultados_operaciones.csv'

# Definir encabezados para archivos CSV
predicciones_headers = ['Hora', 'Predicción de Precio', 'Tendencia de EMA']
metricas_headers = ['Hora', 'RMSE', 'MAE', 'R^2']
resultados_headers = ['Hora', 'Resultado Operación', 'Balance Actual']

# Crear archivos CSV y escribir encabezados
with open(predicciones_file, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(predicciones_headers)

with open(metricas_file, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(metricas_headers)

with open(resultados_file, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(resultados_headers)
...
while operaciones_realizadas < 1000:
    ...
    if entrar:
        ...
        pred_data = [hora_actual, predicted_price, trend_direction]
        metricas_data = [hora_actual, rmse, mae, r2]
        resultado_operacion = ['Hora', 'Resultado Operación', 'Balance Actual'] # Supongo que tendrás que rellenar con los valores adecuados

        analyze_and_trade()

        # Escribir datos en archivos CSV
        write_to_csv(predicciones_file, pred_data)
        write_to_csv(metricas_file, metricas_data)
        write_to_csv(resultados_file, resultado_operacion)
